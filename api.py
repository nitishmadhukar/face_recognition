...
def analyze():
    ...
    for key in resp_obj:
        resp_obj[key] = {'age': resp_obj[key]['age'], 'gender': resp_obj[key]['gender']}
...
def analyzeWrapper(req, trx_id = 0):
    ...
    directory = 'database/analyze'
    for filename in os.listdir(directory):
        instances.append(f"{directory}/{filename}")
    ...
    try:
        resp_obj = DeepFace.analyze(instances, actions = actions, enforce_detection=False)
...
def verify():
    ...
    for key in resp_obj:
        resp_obj[key] = f"Are the images matching? - {resp_obj[key]['verified']}"
...
def verifyWrapper(req, trx_id = 0):
    ...
    directory = 'database/verify'
    for folder in os.listdir(directory):
        instance = []
        instance.append(f"{directory}/{folder}/image1.jpeg")
        instance.append(f"{directory}/{folder}/image2.jpeg")
